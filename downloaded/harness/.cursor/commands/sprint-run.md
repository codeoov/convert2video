...
<!-- sync:cursor-only -->
## Rule Context 주입 (매 루프)

- **Cursor**: `preToolUse(Task)` → `harness-inject-rules.py`
- **Claude**: `PreToolUse(Agent)` → 동일 스크립트 · `UserPromptSubmit` → Harness arm
- **사후 검사**: `run-stop-checks.sh` Rules 1–10 + `harness-extended-stop-checks.py`
<!-- /sync:cursor-only -->
<!-- sync:claude-only
