...
<!-- sync:cursor-only -->
1. 프롬프트 `═══ HARNESS RULE CONTEXT` 블록 = 이번 Round 적용 룰 전문.
2. 없으면 Read `.cursor/harness/cache/latest-evaluator.md` 후 채점.
3. Grill-me·FAIL 사유에 **위반한 RULE 파일명** 인용 (예: `nextjs-security.mdc §mysql2 PS`).
<!-- /sync:cursor-only -->
<!-- sync:claude-only
