...
if (Test-SprintPrompt $prompt) {
    ...
    $out = [PSCustomObject]@{
        ...
        additional_context = "HARNESS ARMED - sprint-harness-gate.mdc`n첫 답변에 Harness Gate 표(5단계)를 넣으세요.`n메인 에이전트는 app/** domain/** data/** feature/** core/** Write/Edit 금지.`n구현: Task(generator). 검증: Task(evaluator) 최소 3라운드, Round 3+ PASS 전 완료 금지.`n예외: 사용자가 「하네스 생략」을 명시한 경우만.`n규칙 번들: .cursor/harness/cache/latest-<role>.md (각 Task에 자동 주입됨)"
    }
