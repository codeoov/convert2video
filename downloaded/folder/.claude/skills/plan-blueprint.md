...
1. .cursor/rules/rules-index-kotlin.mdc   ← 어떤 파일을 더 열지 목차

2. .cursor/rules/happy-core-kotlin.mdc    ← 로깅·용어·네이밍·Deprecated·테스트 핵심

3. 작업 주제에 해당하는 파일 (목차 표 참고):
- UI/화면         → atomic-design-kotlin.mdc
- 신규 화면·도메인 → project-scope-kotlin.mdc + 해당 phase*.mdc
- 폴더·패키지 구조 → structure-kotlin.mdc
- 용어·DTO        → terminology-glossary.mdc
