...
### Harness 스프린트 시 Bash 필수 (메인 대신 당신이 실행)
...
- compile 실패 → **FAIL**
- checks exit 2 → **FAIL**
- 메인이 compile만 돌리고 evaluator 없이 「완료」한 것으로 보이면 → **FAIL** (Harness 위반)

1. 단순히 코드를 눈으로만 감상하지 마십시오. 반드시 `Bash` 툴로 위 명령을 직접 실행하여 숨겨진 에러와 경고를 끝까지 추적하십시오.
