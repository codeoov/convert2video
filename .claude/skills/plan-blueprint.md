...
### Codex Phase Run용 계획 (요청한 경우)

사용자가 페이즈런용 계획 또는 Phase별 자동 순차 실행 준비를 요청하면
`.agents/skills/phase-run/SKILL.md`와 그 coordinator reference의 실행 명세 형식을
읽고, 계획 옆에 `<계획 파일명에서 .md 제거>.phase-run.json` 실행 명세를 함께 만든다.
이미 있는 계획/명세는 먼저 읽고 사용자 작업을 보존한다. 계획 작성은 Task 생성이나
구현 실행 권한이 아니다. 기존 `plan-blueprint.md`가 다른 작업이면 별도 이름을 쓴다.

각 Phase에 안정 ID, 순서, 의존성, 수정 허용 파일, 완료 기준, 실행 모드와 이유,
검증 명령 및 수동 gate를 기록한다. `P1b` 같은 단계도 원래 순서대로 넣는다.
`read-only`는 제품 코드/설정/테스트/규칙 무수정 조사와 기존 검사 실행에 한하며,
나머지는 `harness`가 기본이다. 번호만 보고 P3/P14 등의 하네스를 생략하지 않는다.
필수 수동 검증은 미충족 시 다음 단계로 넘어갈 수 없는 gate로 명시한다.

계획 단계에서 이 구분을 끝내 사용자는 나중에 `페이즈런 <플랜> P0-P14` 한 번으로
시작할 수 있게 한다. 각 Phase의 범위·검증은 구현 직전에 현재 코드와 재대조한다.
