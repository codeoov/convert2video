...
Using the Linear connector, list issues on team `SON` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `SON-NNN-short-slug`, using the issue's real identifier.
...
- `Closes SON-NNN`, using the real Linear issue identifier
