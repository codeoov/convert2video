...
2. `data/`·`ui/`·`video/` 역할 분리 위반 계획이 없는지
3. 전역 공유 유틸·Worker 키·Work 이름 변경이 계획에 없는지 (있으면 Type B 이상)
...
| 수정 파일 전체 경로 목록 | ✅/❌ |
| 각 파일별 변경 요약 | ✅/❌ |
| ConversionWorker/Work 이름·키 영향 여부 | ✅/❌ |
| l10n 문자열 영향 여부 | ✅/❌ |
...
## Type C → Strict-Loop + Storage/Media Security

Planner가 `[Type C: STORAGE/MEDIA]` (또는 레거시 `API/AUTH`) 선언 시:
**Type B 검증 전부 수행 후** 아래 추가:

### ④ 변경 영향 체크
- Room 스키마·Entity 변경 시 → 마이그레이션/버전 bump 계획
- WorkManager 입력 키·unique work 이름 변경 시 → 기존 작업 호환 계획
- 없으면 → **Rejected**

### ⑤ Security / Privacy 체크
- 외부 저장소·MediaStore 쓰기 범위가 최소인지
- URI/파일 경로 전체 로깅 없는지
- 불필요한 네트워크/권한 추가 없는지
- 미흡 항목 → **Rejected**
