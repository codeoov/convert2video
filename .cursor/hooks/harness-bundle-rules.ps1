...
# --- CLAUDE.md 섹션 소스 맵 (포인터 스텁 파일 → CLAUDE.md 섹션 헤딩) ---
$claudemdHeadings = @{
    "convert2video-core.mdc"   = "Core Conventions"
    "do-not-repeat-kotlin.mdc" = "Anti-Repeat / Single-Source Rules"
    "structure-kotlin.mdc"     = "Folder Structure"
    "project-scope-kotlin.mdc" = "Product Scope"
    "terminology-glossary.mdc" = "Terminology Glossary"
}

# CLAUDE.md에서 특정 ## 섹션 본문을 추출하는 헬퍼
# (?sm): s=dotall(개행 포함), m=multiline(^/$가 행 시작/끝 매칭)
# ^## 로 다음 동급 섹션 헤딩을 검출하므로 섹션 사이 빈 줄 수에 무관하게 동작
function Get-ClauseMdSection {
...
foreach ($fname in $fileList) {
    ...
    if ($claudemdHeadings.ContainsKey($fname)) {
        # 포인터 스텁 파일 — CLAUDE.md 섹션에서 실제 본문을 가져온다
        $body = Get-ClauseMdSection -Heading $claudemdHeadings[$fname]
    } else {
        # 일반 규칙 파일 — frontmatter 제거 후 본문 사용
