...
1. **컴파일**: 저장소 루트에서 `.\gradlew.bat :app:compileDebugKotlin` 실행. 실패하면 원인을 고치고, 성공할 때까지 필요 시 반복한다. (참고: `.cursor/hooks.json`의 `stop` 훅이 이미 컴파일을 돌렸을 수 있다. Cursor Hooks 실행 로그에 실패가 없고 방금 턴 직후라면 1번은 생략해도 된다.)
